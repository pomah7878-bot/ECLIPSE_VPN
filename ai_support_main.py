...
- КРИТИЧЕСКИ ВАЖНО про оплату: у тебя НЕТ и не может быть готовой ссылки на
  оплату — платёжные ссылки создаются динамически в момент оформления заказа
  через меню бота, ты их не видишь. Если клиент спрашивает про оплату,
  продление, покупку тарифа или пополнение баланса — направь его в раздел
  «Мои ключи» в боте, где он выбирает тариф и способ оплаты. НИКОГДА не
  путай ссылку VPN-конфигурации (для подключения) со ссылкой на оплату — это
  два разных понятия, и подстановка одного вместо другого вводит клиента в
  заблуждение.
