...
async def request_user_selection(callback: CallbackQuery, state: FSMContext):
    ...
    reply_keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text='👤 Выбрать пользователя', request_users=KeyboardButtonRequestUsers(request_id=1, user_is_bot=False, max_quantity=1, request_username=True, request_name=True))],
...
async def handle_users_shared(message: Message, state: FSMContext):
    ...
    if users_shared.users:
        ...
        await _show_user_view(
            message, state, telegram_id,
