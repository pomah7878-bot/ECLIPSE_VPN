...
def _renew_payment_page_buttons() -> str:
    ...
    return json.dumps([
        ...
        {"id": "btn_renew_pay_qr",      "label": "🏦 Оплатить по СБП",                     "color": "secondary", "row": 4, "col": 0, "is_hidden": False, "action_type": "system", "action_value": None},
    ...
    page_defaults = {
        ...
        'prepayment': {
            ...
            'buttons': json.dumps([
                ...
                {"id": "btn_pay_qr",      "label": "🏦 Оплатить по СБП",                "color": "primary",   "row": 4, "col": 0, "is_hidden": False, "action_type": "system", "action_value": None},
...
def migration_112(conn: sqlite3.Connection) -> None:
    """Версия 112: подпись "Оплата по СБП" -> "Оплатить по СБП" — для
    согласованности с соседней кнопкой "Оплатить картой" (обе теперь в
    форме глагола, а не существительного)."""
    replacements = [
        ("Оплата по СБП", "Оплатить по СБП"),
    ]
...
def migration_123(conn: sqlite3.Connection) -> None:
    """Версия 123: возвращает кнопку "Мои ключи" на экран выдачи/продления
    ключа (key_delivery) — отмена v122. Админ явно подтвердил, что хочет
    видеть её на этом экране отдельным рядом, несмотря на то что "⬅️
    Назад" ведёт туда же."""
    row = conn.execute(
        "SELECT buttons_default, buttons_custom FROM pages WHERE page_key = 'key_delivery'"
    ).fetchone()
    if not row:
        logger.info("Migration v123: страница 'key_delivery' не найдена, пропускаю")
        return

    my_keys_button = {
        "id": "btn_my_keys", "label": "🔑 Мои ключи", "color": "secondary",
        "row": 2, "col": 0, "is_hidden": False, "action_type": "internal", "action_value": "cmd_my_keys",
    }
    ...
    for column_index, column_name in ((0, "buttons_default"), (1, "buttons_custom")):
        ...
        try:
            ...
            continue
        if any(b.get("id") == "btn_my_keys" for b in buttons):
            continue
        # Сдвигаем Назад/На главную на следующий ряд, чтобы Мои ключи встала перед ними
        for b in buttons:
            if b.get("id") in ("btn_key_delivery_back", "btn_back_main"):
                b["row"] = b.get("row", 0) + 1
        buttons.append(my_keys_button)
        conn.execute(
            f"UPDATE pages SET {column_name} = ? WHERE page_key = 'key_delivery'",
            (json.dumps(buttons, ensure_ascii=False),)
        )

    logger.info("Migration v123 applied: кнопка 'Мои ключи' возвращена на экран key_delivery")


def migration_124(conn: sqlite3.Connection) -> None:
    """Версия 124: гарантированно ставит "⬅️ Назад" и "🈴 На главную" на
    экране key_delivery на ОДНУ И ТУ ЖЕ строку (col 0 и col 1), а не
    полагается на относительный сдвиг, как v123 — если фактическое
    расположение кнопок на конкретной инсталляции (buttons_custom) уже
    отличалось от ожидаемого, относительный сдвиг мог не свести их
    вместе. Здесь номер строки выставляется явно и одинаково для обеих,
    ниже последней из остальных кнопок."""
    row = conn.execute(
        "SELECT buttons_default, buttons_custom FROM pages WHERE page_key = 'key_delivery'"
    ).fetchone()
    if not row:
        logger.info("Migration v124: страница 'key_delivery' не найдена, пропускаю")
        return
    ...
    for column_index, column_name in ((0, "buttons_default"), (1, "buttons_custom")):
        ...
        others = [b for b in buttons if b.get("id") not in ("btn_key_delivery_back", "btn_back_main")]
        if not others:
            continue
        target_row = max((b.get("row") or 0) for b in others) + 1

        changed = False
        for b in buttons:
            if b.get("id") == "btn_key_delivery_back" and (b.get("row") != target_row or b.get("col") != 0):
                b["row"], b["col"] = target_row, 0
                changed = True
            elif b.get("id") == "btn_back_main" and (b.get("row") != target_row or b.get("col") != 1):
                b["row"], b["col"] = target_row, 1
                changed = True
